...
Сейчас `/add_dish` и `/update_last_ordered` доступны только администратору.
...
После `/suggest` бот сразу показывает кнопки:
...
После замены пересчитывается только выбранный тип блюда, а текущее выбранное блюдо этого типа исключается из кандидатов, чтобы оно не выпало снова. Для замены берется случайное подходящее блюдо без ограничения `top 3` и без дополнительной сортировки.
